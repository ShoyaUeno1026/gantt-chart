...
CREATE POLICY "ログインユーザーはフィードバックを更新できる"
  ON feedbacks FOR UPDATE TO authenticated
  USING (auth.uid() = user_id OR (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin')
  WITH CHECK (
    -- 管理者はすべてのフィールドを更新可能（is_resolvedを含む）
    (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin'
    -- 非管理者は自分の投稿のみ更新可能、かつis_resolvedはfalseのまま
    OR (auth.uid() = user_id AND is_resolved = false)
  );
CREATE POLICY "ログインユーザーはフィードバックを削除できる"
  ON feedbacks FOR DELETE TO authenticated
  USING (auth.uid() = user_id OR (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin');
